    ...
    ctx.command('music-downloadlink-api/music-downloadlink-星之阁 <keyword:text>', '搜索歌曲并生成语音')
        ...
        if (cfg.imageMode) {
            ...
            const payload = [
                ...
                koishi_1.h.text(`${exitCommandTip.replaceAll('<br />', '\n')}请在 `),
                (0, koishi_1.h)('i18n:time', { value: cfg.waitTimeout }),
                koishi_1.h.text('内，\n'),
                koishi_1.h.text('输入歌曲对应的序号')
            ];
        ...
        const input = await session.prompt((session) => {
            ...
            return koishi_1.h.select(session.elements, 'text').toString();
        }, { timeout: cfg.waitTimeout });
        if ((0, koishi_1.isNullable)(input))
            return `输入超时，已取消点歌。`;
